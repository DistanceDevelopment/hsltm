#                          --------------------------------------
#------------------------- Analytic Variance Estimation functions -----------------------
...
invHessvar <- function(stat,hmmlt,nx4spline=50,doplot=FALSE)
{
  ...
  if(!is.nullmodel(models)) 
    stop("This function only coded for models without covariates.")
  ...
  for(i in 1:vlen){
    ...
    if(is.nan(se)) {
      ...
      x <- seq(b[i]-se/2,b[i]+se/2,length=nx4spline)
  ...
  if(no.se) 
    return(list(invHess=vcv,dstat.db=dstat.db,stat=stat,se=-999,cv=-999))
  else {

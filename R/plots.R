...
{
  if(type!="persp" & type!="both" & type!="contour" & type!="image") 
    stop("Agrument `type' must be `persp', `contour', `image'or `both' (for contour on image).")
  ...
  for(i in 1:n){
    ...
    if(type=="image" | type=="both") 
      image(x,y,h.xy,add=add,xlab=xlab,ylab=ylab)
    if(type=="both") 
      add=TRUE
    if(type=="contour" | type=="both") 
      contour(x,y,h.xy,add=add,nlevels=nlevels,xlab=xlab,ylab=ylab,col=col)
    if(type=="persp") 
      persp(x,y,h.xy,theta=theta,phi=phi,xlab=xlab,ylab=ylab,zlab="Detection hazard")
  }
}
                   ...
                   zlab="pdf of first detections",values=FALSE,...)
{
  ...
  if(logscale) 
    f.xy=log(f.xy)
  if(type=="image" | type=="both") 
    image(x,y,f.xy,add=add,xlab=xlab,ylab=ylab,...)
  if(type=="both") 
    add=TRUE
  if(type=="contour" | type=="both") 
    contour(x,y,f.xy,add=add,nlevels=nlevels,xlab=xlab,ylab=ylab,col=col,...)
  if(type=="persp") 
    persp(x,y,f.xy,theta=theta,phi=phi,xlab=xlab,ylab=ylab,zlab=zlab,...)
  if(values) 
    return(list(x=x,y=y,z=f.xy))
}
...
#' @param allx If TRUE, returns matrix with p(see at y, given x) for each x in data in hmmlt 
#' (dashed gray lines).
                       ...
                       text.cex=0.66)
{
  ...
  if(is.null(ylim)){
    if(allx) 
      ylim <- c(0,max(fyx,barheight))
    else 
      ylim <- c(0,max(f,barheight))  
  }
  
  histline(barheight,breaks=breaks,ylim=ylim,xlab=xlab,ylab=ylab,main=main)
  
  if(allx) 
    for(i in 1:dim(fyx)[2]) 
      lines(ys,fyx[,i],col="gray",lty=2)
  ...
  if(values) 
    return(list(xy=xy,y=ys,fyx=fyx,f=f,breaks=breaks,barheight=barheight))
}
...
#'
#' @description
#' Plots detection function on top of forward detection distance distribution. 
#' Returns a bunch of stuff.
#'  
#' @param hmltm object produced by \code{\link{est.hmltm}}.
#' @param allx If TRUE, returns matrix with p(see at x, given covars) for each x in data 
#' in hmmlt (dashed gray lines).
#' @param values If TRUE, returns function values used in plot.
#' @param breaks histogram break points.
#' @param ylim y-value limits.
#' @param xlab x-axis label.
                       ...
                       type="prob",text.cex=0.66)
{
  ...
  if(type!="density" & type!="prob") 
    stop("Invalid type: must be `density' or `prob'.")
  
  nx <- length(hmmlt$x)
  
  if(is.null(breaks)) 
    h <- hist(hmmlt$xy$x,plot=FALSE)
  else 
    h <- hist(hmmlt$xy$x,plot=FALSE,breaks=breaks)
  ...
  if(is.nullmodel(hmmlt$models)) {
    ...
    for(i in 1:n)
      esw[i] <- sintegral(hmmlt$p[i,],hmmlt$x)
    
    wt <- (1/esw)/sum(1/esw)
    mean.esw <- mean(esw)
    for(i in 1:n) 
      mean.p <- mean.p+hmmlt$p[i,]*wt[i]
    
    #    apply(hmmlt$p,2,mean)
    scaled.density <- h$density*sintegral(mean.p,hmmlt$x)/h.area
    for(i in 1:n) 
      E.density[i,] <- hmmlt$p[i,]*h.area/esw[i]
  ...
  if(type=="prob") {
    ...
    if(is.null(ylim) & allx) 
      ylim <- range(0,scaled.density,hmmlt$p)
    if(is.null(ylim) & !allx) 
      ylim <- range(0,scaled.density,mean.p)
    if(is.null(ylab)) 
      ylab <- "p(x)"
    
    histline(scaled.density,h$breaks,ylim=ylim,xlab=xlab,ylab=ylab)
    
    if(allx & !is.nullmodel(hmmlt$models)) 
      for(i in 1:n) 
        lines(hmmlt$x,hmmlt$p[i,],col="gray")
    ...
    # Count scale
    if(is.null(ylim) & allx) 
      ylim <- range(h$density,E.density)
    if(is.null(ylim) & !allx) 
      ylim <- range(h$density,mean.E.density)
    if(is.null(ylab)) 
      ylab <- "Density"
    ...
    if(allx & !is.nullmodel(hmmlt$models)) 
      for(i in 1:n) 
        lines(hmmlt$x,E.density[i,],col="gray")
  ...
  if(values) 
    return(list(x=hmmlt$x,breaks=breaks,scaled.density=scaled.density,p=hmmlt$p,
                mean.p=mean.p,E.density=E.density,mean.E.density=mean.E.density))
}  
